...
#Geom is the type of graph you are using in ggplot 
...
### ggplot, always put a plus at the end of the line to continue rather than a comma
### if plots are showing up in R markdown document, you can change that using the little wheel 
### next to the knit tab. Tell it to have plot "Chunk Output" show up in console or in R markdown

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)

### good to use "print" function in R markdown because it makes it print in R markdown PDF
### can put aes in either the first line "ggplot" or the "geom_point" line
...
### facet_wrap tells it when to wrap new graph onto a new line -> specifies number of rows or columns 
...
### facet_grid allows to facet across 2 different variables
...
### can have geom_point but also don't need it 
### when do time series, you just want to do one site at a time
### Use NAs in dataset so there is a break in line
...
TempPlot <- ggplot(PeterPaul.chem.nutrients)+
  geom_point(aes(x = daynum, y = temperature_C, color = as.factor(year4)))
print(TempPlot)

TempPlot2 <- ggplot(subset(PeterPaul.chem.nutrients, depth ==0))+
...
TempPlot3 <- ggplot(PeterPaul.chem.nutrients)+
...
# geom_smooth is a line fo best fit
# standard method will curve, can specify "method = lm" linear model 
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? ## fill just colors inside the box
# it doesn't work well because boxes are little and outliers remain black
print(Nutrientplot3)
