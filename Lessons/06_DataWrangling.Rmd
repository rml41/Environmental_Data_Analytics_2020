...
# Indicators of larger climatic changes in the region 
# Harmful algal blooms
# Demonstrate seasonal changes 
# Indicate if excess nutrients are entering the lake 
...
head(NTL.phys.data) # head gives first 6 rows in data frame 
summary(NTL.phys.data)
str(NTL.phys.data) # tells what type of data is in each column 
dim(NTL.phys.data) 
...
class(NTL.phys.data$depth) # meters down from surface

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
### just surface depths (zero)
### must include comma so that it indicates every column 

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
### don't need to specify all rows with dplyr so don't need comma after zero
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) 
    ### useful is you have a long list 
    ### %n% says to include anything that is Paul or Peter
    ### need to concatinate what you want to include

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum) ### shows that samples are taken from 55 - 307th day of the year because can't sample when lake is frozen 
...
> Answer: Because we wanted to fill based on attributes, not row numbers. Would be more work to figure out what rows we want. Also, if data set changes, row numbers change. 
...
### arrange function automatically arranges by ascending 
...
### when we arrange a column, NA will always show up at the bottom of the data set 

# Which dates, lakes, and depths have the highest temperatures?

### East Long Lake and Hummingbird Lake
### Near surfaec depths (0 or 0.5)
### June and July 
...
### as we call out a list of column names, we just use commas 
### select allows you to say you don't want to include something 
### could say sampledate:temperature_C, - lakename
...
### always adds new column on far right side of data
...
### specify data frame as the first line of the code
### all the following functions will run on that data frame
### %>% (pipe) means then 
