...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{### created this chunk by hitting "command, option, i"}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
\begin{quote}
anything in \textless{}\textgreater{} shouldn't appear in R markdown
document
\end{quote}
\end{quote}
\end{quote}
...
\tightlist
\item
  start a list with an "*" and a space
\item
  use italics and bold to highlight words \emph{italic} \textbf{bold}
\end{itemize}

Why R markdown? * Helpful for \textbf{collaboration} *
\textbf{Documentation} of data pipeline steps * Store \emph{code},
\emph{figures}, \emph{graphs}, \emph{tables}, \emph{text} in same
document * \textbf{Organize} code into sections * Aesthetically Pleasing
...
\begin{longtable}[]{@{}ll@{}}
\toprule
Dataset Name & Notes\tabularnewline
\midrule
\endhead
Peter Paul & Cell 2\tabularnewline
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{NTL.Nutrients.processed <-}\StringTok{ }\NormalTok{NTL.Nutrients.raw }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(depth }\OperatorTok{==}\StringTok{ }\DecValTok{0}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(lakename}\OperatorTok{:}\NormalTok{sampledate, depth}\OperatorTok{:}\NormalTok{po4)}
\end{Highlighting}
\end{Shaded}
