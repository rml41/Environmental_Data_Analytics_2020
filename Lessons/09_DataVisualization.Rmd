...
### can do it with a theme, e.g theme_bw() or theme_light()
...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() ### dark line around plot and light gray lines
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() ### gray outline around plot 
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() ### best because most like what we'd see if draw a plot 
print(O3plot4)

### themes don't adjust text size, font, color 
### themes don't adjust any aspect of the legend 
```
...
theme_set(mytheme) 
### if you do this at the beginning of the session,
# all your plots will call up this theme
# look in help function to see all the things you can change in your theme
# can modify components of a theme
...
# b means 3 letter month
 # [3] means subscript 
### ggplot adds things in layers, so need to add things onto your plot on the order
### that you want them to be layered on the plot 
### geom_text - need to specity where to put on plot (x/y coordinate), how to label text
# adjust which way they're justified (hjust)
