...
nc_basins_nf_seplains_wsg84 <- nc_basins_nf_seplains %>%
  st_transform(4326)

nc_gages_wsg84 <- nc_gages %>%
  st_transform(4326)

basins_nf_seplainswsg84 <- basins_nf_seplains %>%
  st_transform(4326)

### basins that overlap with NC 

nc_basins_overlap_nf_seplains <- basins_nf_seplains %>%
  st_overlaps(nc_state_bounds_geom) %>% as.data.frame()

class(nc_basins_overlap_nf_seplains)

st_crs(nc_basins_overlap_nf_seplains)

st_crs(nc_basins_overlap_nf_seplains) <- my_proj4
st_crs(nc_basins_overlap_nf_seplains) <- my_epsg

nc_basins_overlap_nf_seplains_wsg84 <-
   nc_basins_overlap_nf_seplains %>%
  st_transform(4326)


nc_basins_full <- basins_nf_seplains %>%
 nc_basins_overlap_logic(nc_state_bounds_geom)

non_nc_basins <- st_difference(basins_nf_seplains, nc_state_bounds_geom)

class(non_nc_basins)

##map 

pdf(here("outputs", "1b_test_fig.pdf"), width = 11, height = 8.5)
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, inherit.aes = FALSE) +
  geom_sf(data = nc_basins_overlap_nf_seplains_wsg84, alpha = 0.25, inherit.aes = FALSE) +
  geom_sf(data = nc_gages_wsg84 , lwd = 1, inherit.aes = FALSE) +
  theme_bw()
dev.off()


pdf(here("outputs", "1b_test_fig2.pdf"), width = 11, height = 8.5)
ggmap(nc_basemap) +
  geom_sf(data = nc_state_bounds_geom_wsg84, fill = NA, inherit.aes = FALSE) +
  geom_sf(data = basins_nf_seplainswsg84, alpha = 0.25, inherit.aes = FALSE) +
  geom_sf(data = nc_gages_wsg84 , lwd = 1, inherit.aes = FALSE) +
  theme_bw()
dev.off()
