...
### st_read means we are reading a shape file (different than read.csv)
```
...
my_tabular_data_raw <- read_csv(here("data", "tabular_data", "conterm_hydro.txt"))
...
NC_state_bounds <- southeast_state_bounds %>% filter(STUSPS == "NC")

ga_state_bounds_geom <- southeast_state_bounds %>%
  filter(NAME == "Georgia") %>%
  st_geometry()

# check
# add your code here
head(ga_state_bounds_geom)

# select watersheds that intersect with NC bounds
# add your code here

ga_basins_nf_seplains <- basins_nf_seplains %>% 
  st_intersection(ga_state_bounds_geom)
...
ga_area <- st_area(ga_state_bounds_geom)
ga_centroid <- st_centroid(ga_state_bounds_geom)
ga_gages_buffer_100m <- st_buffer (ga_gages, dist = 100)
not_ga_gages <- st_difference(gages, ga_state_bounds_geom)
...
### test GA state boundary

pdf(here("outputs", "ga_state_bounds.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = ga_state_bounds_geom) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()

# plot NC watershed boundardies from 3.1.2
pdf(here("outputs", "spatial_operations_test_fig2.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = ga_state_bounds_geom) +
  geom_sf(data = ga_basins_nf_seplains, fill = "pink", alpha = 0.25)+
  geom_sf(data = ga_gages, color = "black")+
  theme_bw()
dev.off()
...
### st_geometry() drops attributes and just keeps geometry of shape file 
