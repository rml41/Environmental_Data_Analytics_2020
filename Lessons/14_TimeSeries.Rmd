...
Note: time series objects must be equispaced (.e.g need to have a number for every day in dataframe). In our case, we have daily data with no NAs in the data frame, so we don't need to worry about this. We will cover how to address data that are not equispaced later in the lesson.

```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365)

### the number "8" indicates the 8th column 
### Results 
#### so little of the data were captured by the seasonal and trend (see scale bars) so data and remainder look similar 
### Seasonal - a lot of niose in seasonal signal
#### each season looks the same for each of the 35 years because it extracts overall seasonal effect of data and repeats it every 365 days 
...
### specify all the rows in columns 1:3 by putting comma before 1:3 
...
### specify aes in geom_line rather than ggplot so you can have diff aes for each geom
...
wilcox.test(EnoDischarge.early$discharge.mean, 
            EnoDischarge.late$discharge.mean)
...
> There is a significant difference in discharge between the two time periods. 
Mean daily discharge in the Eno River from 1927-1971 was significantly higher than mean daily dischage from 1985-2019 (Wilcoxon test, W=1.17*10^8, p < 0.0001)
Boxplot to show difference in mean discharge 
...
> aggregation: Taking multiple data points and combining them to make one point. For example if you have daily data you could turn it into weekly or monthly to determine trends. Could also be useful if you are missing data, e.g. 28 days of 30 day month, you could take a summary statistic and aggregate data to month. 

> subsampling: For example, if you have daily data, you could take the first of every month as a representative for each month. Used less than aggregation. 

> interpolation: Predicting what could be in data gaps by extrapolating from data. 
...
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year,                                           EnoDischarge.late.monthly$Month, 
                1, sep="-"),
                format = "%Y-%m-%d")
### add this to have a data access for graphing 
### Don't use this column in your actual time series because it's making each monthly average into the 1st day of each month 
...
#### high p value, z close to 0 so we can't see a significant trend
### Z score <-1.96 is decreasing trend 
### Z score > 1.96 is a positive trend 
summary(EnoDischarge.late.trend)
### is there a trend for seasons 
...
> Average monthly discharge from the Eno River has remained consistent from 1985 - 2020. There has been so significant seasonal trend in monthly dischage over the years (Seasonal Mann Kendall, z= -0.14, p = 0.89).  
...
### don't try to graph slope for time series. It's difficult 
