...
```{r}
### created this chunk by hitting "command, option, i"
```
...
>>> anything in <> shouldn't appear in R markdown document 

* start a list with an "*" and a space 
* use italics and bold to highlight words 
*italic* 
**bold**
...
* Store *code*, *figures*, *graphs*, *tables*, *text* in same document 
...
head(NTL.Nutrients.raw)
NTL.Nutrients.raw$sampledate <- as.Date(NTL.Nutrients.raw$sampledate, format = "%m/%d/%y")
...
```{r}
NTL.Nutrients.processed <- NTL.Nutrients.raw %>% 
  filter(depth == 0) %>% 
...
NTL.Nutrients.summary.TN <- NTL.Nutrients.processed %>% 
  group_by(lakename) %>% 
filter(!is.na(tn_ug)) %>% 
summarise(minTN = min(tn_ug),
          meanTN = mean(tn_ug),
          maxTN = max(tn_ug), 
          sdTN = sd(tn_ug))

NTL.Nutrients.summary.TP <- NTL.Nutrients.processed %>% 
  group_by(lakename) %>% 
filter(!is.na(tp_ug)) %>% 
summarise(minTP = min(tp_ug),
          meanTP = mean(tp_ug),
          maxTP = max(tp_ug), 
          sdTP = sd(tp_ug))
...
kable(NTL.Nutrients.summary.TN, col.names = c("Lake","Min", "Mean","Max","Standard Deviation"), caption = "Total Nitrogen Summary Statistics")
kable(NTL.Nutrients.summary.TP, col.names = c("Lake","Min", "Mean","Max","Standard Deviation"), caption = "Total Phosphorus Summary Statistics")
