...
### Explore outliers/anomolies 
### Informs if you need to transform data 
### Informs methods of analysis/statistical tests
### Gives you an overview 
### Helps develop hypotheses -> informs if data set is the data you need to answer questions
### Choose relevant data and create a more streamlined data set
### Make sure it's in the format that you want 
...
### tell R that the date is a date not a factor 
### tell R the date is written at Year, month, date (%Y-%m-%d)

class(USGS.flow.data$datetime)
### after you change a date, always check to make sure R didn't change date to NAs
...
### we want just approved and provisional, but we get 4 bars 
### 4 bars are "blank" "A-approved" "A:e" and "P-provisional" 
### look at just "A" and "P" - notice most are approved 
```
...
### highly skewed near zero because discharges can't be below zero 
### too much negative space to the right 
### could put Y axis on log scale because so many numbers on one side 

# smaller binwidth 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# instead of specifying bin width you can tell ggplot how many bins to make 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# 10 bin width, but decrease the limits on the x axis 
...
### name plot to have it show up in the environment 
gageplot <- ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean))

### print it to show up in markdown PDF 
print(gageplot)
...
ggplot(USGS.flow.data) +
  ...
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "salmon") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) +
  scale_x_continuous(limits = c(0, 10))

### shows that there are more data points for gage height mean than for gage height min and max

###lty means line type. Typical line type is 1, dashed is 2
  ### google line types in R 
###color - there's a helpful PDF if you google "R colors"

# explore how many data points we have in each category 

summary(USGS.flow.data$gage.height.max)
summary(USGS.flow.data$gage.height.min)
summary(USGS.flow.data$gage.height.mean)

### see that there are a lot more NA's for min and manx than for mean 

# what to know if gage heights are different if they are approved or provisional 
### seperate color based on approval code 
### tells ggplot to put legend at top 
ggplot(USGS.flow.data) +
...
### boxplot does not show the large difference in sample size between Approved and Provisional data

#
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

### can specify continuous variable in addition to factor variable as x-axis
### rarely used 
...
### shows sample count information better 
### if you have more data in a specific part of data, it will be wider 
### can draw quartiles in 
...
> Answer: Depending on counts, skew, outliers, distribution 

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: More data points for mean than for min and max, gaps in data (1970s-1980s)


How can multiple options for data exploration inform our understanding of our data?

> Answer: Tell us number of data points in each category, tell us skew and distribution 
...
> Answer: Correlation between mean gage height and mean dischage 
