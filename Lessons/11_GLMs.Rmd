...
### no longer split by functional group
...
### aov function: tell it what data we are pulling from, and what relationship we are testing 
##### are the means of dryMass different by plotID?
### results show plot ID is a significant indicator of dryMass value 
### important to have a lot of additional degrees of freedom for ANOVA (residuals)

# Format ANOVA as lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)

### lm shows the estimate for the difference in mean for each individual group 
### intercept is just the first value in the dataset
### pvalue shows whether alpha is significantly different than the intercept (first option)
### output gives R squared, but we don't report this for ANOVA
...
### Tukey only works on the aov object
### gives you all the pairs, sometimes it can be too much to sort through 
### use HSD to extract groups

# Extract groupings for pairwise relationships
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups

### if one plot ID matches letter from another plot ID, it is not significantly different 
...
### stat_summary - tell what geometery you want "text",
### where to place it "fun.y =max", and labels (must be specific order in 
### which they appear in dataset (not how they appear in HSD))
### might need to reset ylim for stat summary so text isn't cut off 
### vjust = -1 tells "abcd" to go up above points not on top of them 
```

What information does the output give us? How might we report this information in a report?

> Total litter biomass differed significantly among sites at the Niwot Ridge LTER (figure 1: ANOVA, F11,198 = 4.813, p<0.0001)
...
### Results: functional group in general is a significant predictor 
### Results: nlcdClass in general is a significant predictor
...
### Intercepts is first group alphabetically in both functionalGroup and nlcdClass
...
### change + nlcdClass to *nlcdClass
# Format as aov
Litter.anova.2way3 <- aov(data = Litter, dryMass ~ functionalGroup * nlcdClass)
summary(Litter.anova.2way3)

### p value <0.05 shows there is a significant interaction between the two variables
...
### above need to define litter interaction 
########(find interaction between functionalGroup and nlcdClass)
Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction)
### allows you to use litter interaction in HSD test
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  ...
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
               ...
               show.legend = FALSE) +
  ylim(0,50)+
  labs(x = "Class", y = "Dry Mass (g)", color = "") 
