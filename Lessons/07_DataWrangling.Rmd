...
### need to load plyr before tidyverse because they have conflicting verbs 
### the last package that is loaded, masks the previous package
```
...
### filter function still includes all the information of other factor levels even if you filter them out 
...
### drop_na function takes out rows with NA value in specified column 
### good because can use specific column, rather than remove all NAs
...
### just need to define which one will be column names and which will be value of column (first column is column name (nutrient) secone is column values (concentration))
...
### seperate does not retain original column 
...
###ldply function combines results into 1 df
```
