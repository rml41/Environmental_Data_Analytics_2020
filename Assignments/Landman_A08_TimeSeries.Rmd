...
GaringerOzone<- 
  rbind(GaringerOzone2010, GaringerOzone2011, GaringerOzone2012,
...
ggplot(GaringerOzone, 
  ...
  geom_line(size = 0.5)+
  labs( x = "Date", y = "Daily Max 8 Hours Ozone Concentration (ppm)")
...
> Answer: Linear interpolation fits a straight line between two data points to determine the values of interpolation. This works well in this situation because ozone concentration betwen two days will most likely be somewhere between the previous and the following day. There is no need for a quadratic function as spline would use and connecting it to the nearest point using piecewise would be difficult because a lot of missing data will have two equally near points on either side and it wouldn't reflect the actual trend.  
...
> Answer: Our initial graph of ozone over time looked like it has a seasonal cycle, with high variablily. Therefore, it is valuable to account for the seasonal cycle when assessing if ozone concentrations have a monotonic trend. 
...
GaringerOzone.monthly.ts <- 
  ts(GaringerOzone.monthly$Ozone, frequency = 12,
...
sens.slope(GaringerOzone.monthly.ts)
...
print(GaringerOzone.monthly.trend)
...
> Answer: Average monthly ozone concentration has decreased at a rate of 0.00025 ppm per year from 2010 - 2020. There has been no significant seasonal trend in ozone concentration over the years (Seasonal Mann Kendall, z= -1.963, p = 0.0497).  
