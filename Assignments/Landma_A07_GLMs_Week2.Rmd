...
TempAIC <- lm(data = NTL.chem.July, 
              temperature_C ~ year4 + daynum + depth)

step(TempAIC)

TempRegression <- lm(data = NTL.chem.July, 
                     temperature_C ~ year4 + daynum + depth)
...
variables that predict temperature. The model explains 74% of the observed variance. 

6. Run an interaction effects ANCOVA to predict temperature based on depth and lakename from the same wrangled dataset.

```{r}
#6
Tempancova.interaction <- lm(data = NTL.chem.July, temperature_C ~ lakename * depth)
summary(Tempancova.interaction)
...
> Answer: There is a significant interaction between depth and lake name. This model explained 79% of variance in temperature. 
...
Tempplot <- ggplot(data = NTL.chem.July, aes(x = depth, y = temperature_C, color = lakename)) + 
...
> Answer: Yes, the mixed effects model is a better model than the fixed effects model. This is apparent because the AIC for the mixed effect model is lower than the AIC for the fixed effects model. Additionally, the p-value <.0001, which indicates there is a significant difference between the models. 